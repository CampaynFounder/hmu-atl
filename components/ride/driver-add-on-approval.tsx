...
export default function DriverAddOnApproval({ rideId, addOns, agreedPrice, onUpdated }: Props) {
  ...
  return (
    <div style={{
      ...
      {/* Removal requests — need approval */}
      {removalPending.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 10, color: C.red, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
            Removal requests
          </div>
          {removalPending.map(a => (
            <div key={a.id} style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '6px 0',
              borderBottom: '1px solid rgba(255,255,255,0.04)',
            }}>
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: 13, color: C.grayLight, textDecoration: 'line-through' }}>{a.name}</span>
              </div>
              <span style={{ fontSize: 12, color: C.gray, flexShrink: 0 }}>${Number(a.subtotal || 0).toFixed(2)}</span>
              <button
                onClick={() => handleAction(a.id, 'confirm_removal')}
                disabled={acting === a.id + 'confirm_removal'}
                style={{
                  background: 'rgba(255,82,82,0.15)', color: C.red, border: 'none', borderRadius: 100,
                  padding: '4px 8px', fontSize: 10, fontWeight: 700, cursor: 'pointer', flexShrink: 0,
                }}
              >
                {acting === a.id + 'confirm_removal' ? '...' : 'Remove'}
              </button>
              <button
                onClick={() => handleAction(a.id, 'reject_removal')}
                disabled={acting === a.id + 'reject_removal'}
                style={{
                  background: 'rgba(255,255,255,0.06)', color: C.grayLight, border: 'none', borderRadius: 100,
                  padding: '4px 8px', fontSize: 10, fontWeight: 700, cursor: 'pointer', flexShrink: 0,
                }}
              >
                {acting === a.id + 'reject_removal' ? '...' : 'Keep'}
              </button>
            </div>
          ))}
        </div>
      ...
      {/* Confirmed items — driver can see and remove */}
      {confirmedItems.length > 0 && (
        <div>
          {(pendingItems.length > 0 || removalPending.length > 0) && (
          ...
          {confirmedItems.map(a => (
            <div key={a.id} style={{
              ...
              <span style={{ flex: 1, fontSize: 12, color: C.grayLight }}>{a.name}</span>
              <span style={{ fontSize: 12, color: C.green, fontWeight: 600, flexShrink: 0 }}>
                ${Number(a.subtotal || 0).toFixed(2)}
              </span>
              <button
                onClick={() => handleDriverRemove(a.id)}
                disabled={acting === a.id + 'driverRemove'}
                style={{
                  background: 'none', border: 'none', color: C.gray, fontSize: 10, cursor: 'pointer',
                  padding: '2px 4px', flexShrink: 0,
                }}
              >
                {acting === a.id + 'driverRemove' ? '...' : '✕'}
              </button>
            </div>
          ))}
        </div>
