...
export default function RiderFeedClient({ displayName, userId }: Props) {
  ...
  // Browse-funnel drafts now flow auth-callback → /rider/booking-sent (PM on
  // file) or /onboarding express PM-only mode (no PM). The rider only lands
  // on /rider/home for this flow if a stale legacy localStorage entry is
  // sitting around — clear it defensively so it never re-fires.
  useEffect(() => {
    try {
