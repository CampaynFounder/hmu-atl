...
export async function GET() {
  ...
  const blastRequests = blastRows.map((row: Record<string, unknown>) => {
    ...
    return {
      ...
      time: timeLabel,
      stops: (row.stops as Array<{ lat: number; lng: number; address?: string }>) || [],
      roundTrip: (row.trip_type as string) === 'round_trip',
  ...
  const requests = rows.map((row: Record<string, unknown>) => {
    ...
    return {
      ...
      dropoffAreaSlug: (row.dropoff_area_slug as string) || null,
      dropoffInMarket: row.dropoff_in_market !== false,
      destination: (row.dropoff_address as string) || (tw.destination as string) || (tw.dropoff as string) || (tw.message as string) || (tw.note as string) || '',
      pickupAddress: (row.pickup_address as string) || (tw.pickup as string) || '',
      time: tw.time ?? '',
      stops: Array.isArray(tw.stops) ? (tw.stops as Array<{ lat: number; lng: number; address?: string }>) : [],
      roundTrip: (row.post_type === 'direct_booking'
        ? (tw.round_trip === true || (row.trip_type as string) === 'round_trip')
        : (tw.round_trip === true)),
      price: Number(row.price ?? 0),
