...
// VAPID configuration — set VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, VAPID_SUBJECT in env
...
/**
 * Store a push subscription for a user in Neon.
 * Upserts on endpoint to avoid duplicates.
 */
 ...
 * Uses Redis dedup key — skips if same (userId + tag) sent within 60 s.
...
export async function sendPush(
  ...
  if (!already) return 0; // duplicate — skip
