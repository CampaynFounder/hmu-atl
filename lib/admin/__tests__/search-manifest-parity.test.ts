...
import { PERMISSION_SECTIONS } from '@/app/admin/roles/permission-matrix';
...
describe('permission slug parity', () => {
  it('every permission-gated route slug is assignable in the role matrix', () => {
    const matrixKeys = new Set(
      PERMISSION_SECTIONS.flatMap((g) => g.items.map((i) => i.key)),
    );
    const missing = ADMIN_ROUTES
      .filter((e) => e.rule.kind === 'permission')
      .map((e) => (e.rule as { slug: string }).slug)
      .filter((slug) => !matrixKeys.has(slug));
    // Dedup for a clean message.
    const unique = [...new Set(missing)];
    expect(unique, `Route slugs not in the role matrix (unassignable): ${unique.join(', ')}`).toEqual([]);
  });

  it('route slugs are base slugs, not pre-suffixed with an access level', () => {
    // canAccess() appends `.view`; a slug ending in .view/.edit/.publish double-
    // suffixes and can never match a real grant (the old /admin/blast bug).
    const suffixed = ADMIN_ROUTES
      .filter((e) => e.rule.kind === 'permission')
      .map((e) => (e.rule as { slug: string }).slug)
      .filter((slug) => /\.(view|edit|publish)$/.test(slug));
    expect(suffixed, `Route slugs wrongly suffixed with a level: ${suffixed.join(', ')}`).toEqual([]);
  });
});
