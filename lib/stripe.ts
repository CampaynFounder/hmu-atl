 ...
 * When STRIPE_MOCK=true all calls are simulated locally.
 * Wire real Stripe by setting STRIPE_MOCK=false and providing STRIPE_SECRET_KEY.
 */
...
async function getRealStripe(): Promise<any> {
  // Dynamic import so the build never fails even without the stripe package installed.
  // Install stripe and set STRIPE_MOCK=false to activate real transfers.
