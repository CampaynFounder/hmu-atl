...
-- via the existing /api/drivers/[handle]/book route. 60-min TTL covers
-- signup + onboarding (name → media → PM), with headroom for video upload.
...
-- For databases where the table already exists with the old default, bump it.
ALTER TABLE public_draft_bookings
