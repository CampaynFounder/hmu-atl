// Rider ride history — mirrors driver rides.tsx structure.
// Rating pills use string codes, not numeric stars.
    ...
    <Animated.View style={{ opacity: fadeAnim }}>
      <TouchableOpacity style={[r.row, shadow.card]} onPress={onPress} activeOpacity={0.8}>
        <View style={r.top}>
          ...
          <Text style={[r.status, { color: statusMeta.color }]}>{statusMeta.label}</Text>
        </View>

        <View style={r.meta}>
          <Text style={r.metaText}>{driverLabel}</Text>
          <Text style={r.dot}>·</Text>
          <Text style={r.metaText}>{formatDate(ride.created_at)}</Text>
          <Text style={r.dot}>·</Text>
          <Text style={r.price}>
            {ride.is_cash ? '💵 ' : ''}${price.toFixed(2)}
          </Text>
        </View>
...
export default function RiderRides() {
  ...
  function openRide(id: string) {
    router.push(`/(rider)/ride/${id}` as any);
  }
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RideRow
              ride={item}
              onPress={() => openRide(item.id)}
              onRateNow={() => openRide(item.id)}
            />
          )}
          ListEmptyComponent={
            <View style={s.empty}>
...
const s = StyleSheet.create({
  ...
  emptyText: { fontFamily: fonts.mono, fontSize: 13, color: colors.textFaint, letterSpacing: 1 },
});
...
const r = StyleSheet.create({
  ...
  status: { fontFamily: fonts.mono, fontSize: 10, letterSpacing: 1 },
  meta: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginBottom: 10 },
