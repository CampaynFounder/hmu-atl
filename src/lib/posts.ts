...
import { redis, DRIVER_POST_PREFIX, RIDER_POST_PREFIX, AREA_DRIVERS_PREFIX, AREA_RIDERS_PREFIX } from './redis';
...
export async function saveDriverPost(post: DriverPost): Promise<void> {
  ...
  const key = `${DRIVER_POST_PREFIX}${post.id}`;
  await redis.set(key, post, { ex: ttlSeconds });

  // Index by area (sorted set — score = expiry unix timestamp)
...
export async function saveRiderPost(post: RiderPost): Promise<void> {
  const ttlSeconds = post.time_window * 60;
  const key = `${RIDER_POST_PREFIX}${post.id}`;
  await redis.set(key, post, { ex: ttlSeconds });
...
/** Get active driver posts for an area, pruning expired ones */
export async function getActiveDriverPosts(
  ...
  // Remove expired members from the index
...
/** Get active rider posts for an area, pruning expired ones */
...
/**
 * Try to match a new rider post against active driver posts in the same area.
 * Returns the matched pair or { matched: false }.
 */
export async function matchRiderToDriver(
  riderPost: RiderPost
): Promise<MatchResult> {
  ...
  for (const driver of drivers) {
    ...
    // Persist updates (keep remaining TTL by recalculating)
    const driverTtl = Math.max(
      1,
      Math.floor((Date.parse(driver.expires_at) - Date.now()) / 1000)
    );
    const riderTtl = Math.max(
      1,
      Math.floor((Date.parse(riderPost.expires_at) - Date.now()) / 1000)
    );

    await redis.set(`${DRIVER_POST_PREFIX}${driver.id}`, updatedDriver, {
      ex: driverTtl,
    });
    await redis.set(`${RIDER_POST_PREFIX}${riderPost.id}`, updatedRider, {
      ex: riderTtl,
    });
