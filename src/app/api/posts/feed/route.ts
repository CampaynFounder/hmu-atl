...
export async function GET(req: NextRequest) {
  ...
  if (!area) {
    return NextResponse.json({ error: 'Missing required query param: area' }, { status: 400 });
  }
  ...
  return NextResponse.json({ area, drivers, riders, total: drivers.length + riders.length });
}
