...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!rows.length) return NextResponse.json({ error: 'User not found' }, { status: 403 });
  ...
  if (!subscription?.endpoint || !subscription?.keys?.p256dh || !subscription?.keys?.auth) {
    return NextResponse.json({ error: 'subscription with endpoint and keys (p256dh, auth) is required' }, { status: 400 });
  }
